    ...
    >
      ...
      <div className="relative p-3 rounded-full bg-card border-2 border-border shadow-lg hover:shadow-2xl transition-all hover:scale-110 backdrop-blur-sm">
        <div className="relative w-6 h-6">
          {/* Sun Icon */}
          <Sun
            className={`absolute inset-0 w-6 h-6 text-amber-500 transition-all duration-500 ${isDark
          ...
          {/* Moon Icon */}
          <Moon
            className={`absolute inset-0 w-6 h-6 text-blue-500 transition-all duration-500 ${isDark
              ? 'rotate-0 scale-100 opacity-100'
              : '-rotate-90 scale-0 opacity-0'
              }`}
          />
