...
router.post("/admin/incubated-startups", authMiddleware, upload.single("image"), async (req, res) => {
  try {
    ...
    const newStartup = new IncubatedStartup(startupData);
...
router.put("/admin/incubated-startups/:id", authMiddleware, upload.single("image"), async (req, res) => {
  try {
    ...
    const updated = await IncubatedStartup.findByIdAndUpdate(
      req.params.id,
      updateData,
...
// GET all incubated startups - PUBLIC (WITH image data as base64)
